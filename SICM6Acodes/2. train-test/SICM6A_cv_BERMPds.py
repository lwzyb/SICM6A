...
findseqlen=501
batchnum=1000
cvdspath="C:/M6Acv/Full_transcript/cv/" 
savepath="C:/M6Acv/Full_transcript/cv/"
...
#savepath="C:/M6Acv/Full_transcript/Tgru/"
###############################
####### 2. Mature_mRNA #########
#specieID=2
#seqlen=251
...
#traindatapath="C:/M6Acv/DATA/BERMP/Full_transcript training data.txt"
#testdatapath="C:/M6Acv/DATA/BERMP/Full_transcript testing data.txt" 
#cvdspath="C:/M6Acv/Mature_mRNA/cv/" 
#savepath="C:/M6Acv/Mature_mRNA/cv/"
#savepath="C:/M6Acv/Mature_mRNA/cv/"
###############################  
####### 3. A.thaliana #########
#specieID=3
...
#findseqlen=101
#batchnum=200
#cvdspath="C:/M6Acv/A.thaliana/cv/" 
#savepath="C:/M6Acv/A.thaliana/cv/"
###############################  
####### 4. S.cerevisiae #########
#specieID=4
#seqlen=51
...
#cvdspath="C:/M6Acv/S.cerevisiae/cv/" 
#savepath="C:/M6Acv/S.cerevisiae/cv/"
############################### 
...
class SICNet(nn.Module):
   ...
   def __init__(self ):   
        ...
        self.LSTM3_1 =nn.GRU(input_size=125,hidden_size=125 , 
                     num_layers=2,  dropout=0.5, batch_first=True   )  
        
        self.LSTM3_2 =nn.GRU(input_size=findseqlen,hidden_size=128 ,
                             num_layers=2,  dropout=0.5, batch_first=True    )  
  
        self.LSTM3_3 =nn.GRU(input_size=16 ,hidden_size=64,
                          num_layers=2,  dropout=0.5, batch_first=True   )

        
        self.fc_1= nn.Sequential( nn.Linear(64,16) ,  
...
if __name__ == "__main__":
    ...
    cvnum=5
    repeatnum=5
    saveauchead(savepath+str( cvnum)+"/auclist.txt"  )
    for i in range(repeatnum):
        y_test_com_full =[]
        y_predicted_com_full =[] 
        for j in range(  cvnum): 
            #traindatapath="C:/M6Acv/DATA/BERMP/S.cerevisiae_train.txt"
            #testdatapath="C:/M6Acv/DATA/BERMP/S.cerevisiae_test.txt" 
            seq_dict_train , seq_label_train = readseqtodict(cvdspath+str( cvnum)+"/"+str( i)+"/"+str(j)+"_train.txt")
            seq_dict_test , seq_label_test = readseqtodict(cvdspath+str( cvnum)+"/"+str( i)+"/"+str(j)+"_predict.txt")
            ...
            for epoch in range( epochnum):
                ...
                if   epoch>=0:
                    ...
                    modelfile=savepath+str( cvnum)+"/"+str( i)+"/"+"Full_transcript_cv_"+str( cvnum)+"_r"+str( i)+"_cv"+str(j)+"_"+str(vaucint)+"_"+str( epoch)+ ".pth"
                    torch.save(model.state_dict() ,modelfile )
                    saveprobdata(savepath+str( cvnum)+"/"+str( i)+"/"+"Full_transcript_cv_"+str( cvnum)+"_r"+str( i)+"_cv"+str(j)+"_"+str(vaucint)+"_"+str( epoch)+ "_prob.txt", np_data_full)                
                    model.train()
                    model.zero_grad()
                    print(  auclist )    
            
            if len(y_test_com_full)<=0:
                y_test_com_full=y_test_com
            else:
                y_test_com_full=np.vstack(            (y_test_com_full,y_test_com))
            if len(y_predicted_com_full)<=0:
                y_predicted_com_full=y_predicted_com
            else:
                y_predicted_com_full=np.vstack((y_predicted_com_full,y_predicted_com)  )  
        v_auc=ROC(y_test_com_full,y_predicted_com_full)
        np_data_full=np.hstack((y_test_com_full,y_predicted_com_full))
        #np.savetxt("C:/M6Acv/S.cerevisiae/"+str( cvnum)+"/"+str( i)+"/prob_full.txt", np_data_full)
        saveprobdata(savepath+str( cvnum)+"/"+str( i)+"/prob_full.txt", np_data_full)
        saveauc(savepath+str( cvnum)+"/auclist.txt", cvnum,i, 51, v_auc)
